...
def getCandidateInfoList(requireOnDisk_bool=True):
    ...
    # the subsets yet.
    mhd_list = glob.glob('Users/kweiss/git/my_LUNA/data/subset*/*.mhd')
    presentOnDisk_set = {os.path.split(p)[-1][:-4] for p in mhd_list}

    candidateInfo_list = []
    with open('Users/kweiss/git/my_LUNA/data/annotations_with_malignancy.csv', "r") as f:
    ...
    with open('Users/kweiss/git/my_LUNA/data/candidates.csv', "r") as f:
        for row in list(csv.reader(f))[1:]:
            series_uid = row[0]
...
class Ct:
    def __init__(self, series_uid):
        mhd_path = glob.glob(
            'Users/kweiss/git/my_LUNA/data/subset*/{}.mhd'.format(series_uid)
        )[0]
